...
const SubscriptionItem = (props) => {
  ...
  const {width, height, bgImg, page, subscription, reviewConnect, isDetail} = props
...
return(
  <ImgDiv width={width} height={height} onClick={onClick}>
      ...
      <div style={textStyle({isHover})}     
        ...
        <div>
          <div style={titleStyle}>{subscription.subscriptionsName} <span style={subTitleStyle}>[ {onGoing} ]</span></div>
        </div>
        <FlexBox justify='space-between' align='end'>
          <FlexBox direction='column' align='start'>
            <div style={detailStyle}>구독 ID: {subscription.subscriptionPk}</div>
            <div style={detailStyle}>구독 기간 {subscription.startDate.replaceAll('-','.')} ~ {subscription.endDate.replaceAll('-','.')}</div>
          </FlexBox>
          {reviewConnect? 
